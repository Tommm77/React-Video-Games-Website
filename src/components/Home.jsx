...
/**
 * Home component to display a list of games.
 *
...
const Home = () => {
  ...
  /**
   * Fetches games from the API based on the specified page.
   *
    ...
    axios
      ...
      .then((response) => {
        ...
        Promise.all(gamePromises).then((gameResponses) => {
          const detailedGames = gameResponses
            .filter((gameResponse) => gameResponse !== null)
            .map((gameResponse) => gameResponse.data);
          const uniqueGames = Array.from(
            new Set([...games, ...detailedGames].map((game) => game.id))
          ).map((id) =>
            [...games, ...detailedGames].find((game) => game.id === id)
          );
  ...
  /**
   * Loads more games by incrementing the current page number.
   */
  const loadMoreGames = () => {
